...
export default function GameProgressChart({ userId }: { userId: string }) {
  ...
  // Доступные типы графиков: прогресс и детализация (столбики)
  ...
  const colors = {
    challenge1: "#3b82f6", // blue
    challenge2: "#10b981", // emerald
    challenge3: "#f59e0b", // amber
    challenge4: "#ef4444", // red
    score: "#8b5cf6",      // violet
    time: "#f97316",       // orange
  };

  const chartTypes = [
    { key: "progress", label: "Прогресс", desc: "Общий балл и время" },
    { key: "details", label: "Детализация", desc: "Результаты по испытаниям" },
  ] as const;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-4 border border-purple-200">
          ...
          <div className="text-3xl font-bold text-purple-800">{analytics.bestScore.toFixed(1)}</div>
          <div className="text-xs text-purple-600 mt-1">
            Худший: {analytics.worstScore.toFixed(1)}
          </div>
        </div>
        <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg p-4 border border-orange-200">
          <div className="text-sm text-orange-600 font-medium">Лучшее время</div>
          <div className="text-3xl font-bold text-orange-800">
            {analytics.bestTime.toFixed(1)}с
          </div>
          <div className="text-xs text-orange-600 mt-1">
            Среднее: {analytics.averageTime.toFixed(1)}с
          </div>
        </div>
      </div>
        ...
        {Object.entries(analytics.challengeBest).map(([challenge, value]) => (
          <div key={challenge} className="bg-white rounded-lg p-3 border">
            ...
            <div className="text-xs text-gray-500">
              Среднее: {analytics.challengeAverages[challenge as keyof typeof analytics.challengeAverages].toFixed(1)}
            </div>
          </div>
        ))}
      </div>
                      ...
                      return (
                        <div className="bg-white p-4 border rounded-lg shadow-lg">
                          ...
                          <div className="mt-2 space-y-1">
                            <p className="text-purple-600">Баллы: {typeof data.score === 'number' ? data.score.toFixed(1) : data.score}</p>
                            <p className="text-orange-600">Время: {typeof data.time === 'number' ? data.time.toFixed(1) : data.time}с</p>
                            <p className="text-blue-600">Исп.1: {typeof data.challenge1 === 'number' ? data.challenge1.toFixed(1) : data.challenge1}</p>
                            <p className="text-green-600">Исп.2: {typeof data.challenge2 === 'number' ? data.challenge2.toFixed(1) : data.challenge2}</p>
                            <p className="text-yellow-600">Исп.3: {typeof data.challenge3 === 'number' ? data.challenge3.toFixed(1) : data.challenge3}</p>
                            <p className="text-red-600">Исп.4: {typeof data.challenge4 === 'number' ? data.challenge4.toFixed(1) : data.challenge4}</p>
                          </div>
                        </div>
                      );
                    }
                      ...
                      return (
                        <div className="bg-white p-4 border rounded-lg shadow-lg">
                          ...
                          <div className="mt-2 space-y-1">
                            ...
                            <p className="text-yellow-600">Исп.3: {typeof data.challenge3 === 'number' ? data.challenge3.toFixed(1) : data.challenge3}</p>
                            <p className="text-red-600">Исп.4: {typeof data.challenge4 === 'number' ? data.challenge4.toFixed(1) : data.challenge4}</p>
                            <p className="border-t pt-1 mt-1 font-bold text-purple-600">Всего: {typeof data.score === 'number' ? data.score.toFixed(1) : data.score}</p>
                          </div>
                        </div>
                      );
                    }
              ...
              {filteredGames.map((game) => (
                ...
                >
                  ...
                  <td className="py-2 px-2">
                    {typeof game.challenge1 === 'number' ? game.challenge1.toFixed(1) : game.challenge1}
                  </td>
                  <td className="py-2 px-2">
                    {typeof game.challenge2 === 'number' ? game.challenge2.toFixed(1) : game.challenge2}
                  </td>
                  <td className="py-2 px-2">
                    {typeof game.challenge3 === 'number' ? game.challenge3.toFixed(1) : game.challenge3}
                  </td>
                  <td className="py-2 px-2">
                    {typeof game.challenge4 === 'number' ? game.challenge4.toFixed(1) : game.challenge4}
                  </td>
                  <td className="py-2 px-2">
                    <span className={`font-bold ${
                      game.score === analytics.bestScore ? "text-green-600" : ""
                    }`}>
                      {typeof game.score === 'number' ? game.score.toFixed(1) : game.score}
                      {game.score === analytics.bestScore && " (Рекорд)"}
                    </span>
                  </td>
                  <td className="py-2 px-2">
                    {typeof game.time === 'number' ? game.time.toFixed(1) : game.time}с
          ...
          onClick={() => {
            const csv = [
              ...
              ...analytics.allGames.map(g => 
                [g.gameNumber, g.date, g.challenge1, g.challenge2, g.challenge3, g.challenge4, g.score, g.time].join(",")
              )
            ].join("\n");
